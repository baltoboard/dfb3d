...
static Tri3D* Triangles = NULL;
static Vertex* TransformedVerticies = NULL;
...
static void LoadIt (char* Obj3dFilename)
{
  if (LoadObj3d(Obj3dFilename) == 0) {
    ScaleFactor = 0.25 / Obj3dMaxCoordinate;
    Triangles = calloc(Obj3dTrianglesCount, sizeof(Tri3D));
    TransformedVerticies = calloc(Obj3dVerticiesCount, sizeof(Vertex));
  } else {
    Obj3dTriangles = SkullTriangles;
    Obj3dVerticies = SkullVerticies;
    Obj3dTrianglesCount = SKULL_TRIANGLES;
    Obj3dVerticiesCount = SKULL_VERTICIES;
    Triangles = calloc(Obj3dTrianglesCount, sizeof(Tri3D));
    TransformedVerticies = calloc(Obj3dVerticiesCount, sizeof(Vertex));
  }
}
